...
export const ConflictResolutionModal: React.FC<ConflictResolutionModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md animate-in fade-in">
      <div className="w-full max-w-lg glass-card p-6 md:p-8 rounded-3xl border border-amber-500/30 bg-[#131024]/95 shadow-2xl space-y-6">
        {/* Header */}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-end gap-3 pt-2">
          <button
            type="button"
            onClick={onCancel}
            className="w-full sm:w-auto px-4 py-2.5 rounded-xl border border-purple-500/20 text-xs font-semibold text-purple-200 hover:bg-purple-500/10 cursor-pointer"
          >
            Cancelar
          </button>
          <button
            type="button"
            onClick={onUseServer}
            className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-[#241e3d] text-amber-300 hover:bg-amber-500/20 text-xs font-bold border border-amber-500/30 cursor-pointer"
          >
          ...
          <button
            type="button"
            onClick={onKeepLocal}
            className="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 text-white hover:opacity-95 text-xs font-bold shadow-lg shadow-purple-900/40 cursor-pointer"
          >
            Manter Minha Edição
          </button>
        </div>
